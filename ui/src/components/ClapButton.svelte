<script lang="ts">
  ...
  async function sendClaps(claps: number) {
    const data = await fetch('/addClaps.json', {
      method: 'PUT',
      headers: {
        'Content-Type': 'application/json',
      },
      body: JSON.stringify({ id, claps }),
    })
  ...
  function clap() {
    ...
    timeout = setTimeout(async () => {
      const totalClaps = claps + clapQueue
      clapQueue = 0
      const savedClaps = await sendClaps(totalClaps)
      claps = savedClaps
    }, 2000)
  }
</script>
...
<style>
  ...
  button:active {
    box-shadow: inset 1px 1px 5px var(--text-color);
  }
  .added-claps {
    top: 0;
...
<div>
  {#if clapQueue}
    <span class="added-claps" transition:fly={{ y: 100, duration: 1000 }}>
      +{clapQueue}
    </span>
  {/if}
  <button on:click={clap}>
    <svg xmlns="http://www.w3.org/2000/svg" viewBox="-5 -2 38 36">
      <path
  ...
  <span class="current-claps">{claps} claps</span>
</div>
